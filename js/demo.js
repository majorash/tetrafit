...
Demo = {
  ...
  sort: {
    ...
    area    : function (a,b) { return Demo.sort.msort(a, b, ['type','a', 'h', 'w']);          },
    maxside : function (a,b) { return Demo.sort.msort(a, b, ['type','max', 'min', 'h', 'w']); },
    type_ : function (a,b) { return Demo.sort.msort(a, b, ['type']); },
  ...
  blocks: {
    ...
    examples: {
        ...
        complex: [
        {w: 100, h: 100, type: 0 ,num:   3,},
        {w:  60, h:  60, type: 0, num:   3},
        {w:  50, h:  20, type: 2, num:  20},
        {w:  20, h:  50, type: 1, num:  20},
        {w: 250, h: 250, type: 1, num:   1},
        {w: 250, h: 100, type: 0, num:   1},
        {w: 100, h: 250, type: 0, num:   1},
        {w: 400, h:  80, type: 2, num:   1},
        {w: 80,  h: 400, type: 0, num:   1},
        {w:  10, h:  10, type: 1, num: 100},
        {w:   5, h:   5, type: 2, num: 500}
      ],
    ...
    deserialize: function(val) {
      ...
      for(i = 0 ; i < blocks.length ; i++) {
        ...
        if (block.length >= 3)
          result.push({w: parseInt(block[0]), h: parseInt(block[1]), num: (block.length == 3 ? 1 : parseInt(block[3])),type: parseInt(block[2]) });
      }
      var expanded = [];
      for(i = 0 ; i < result.length ; i++) {
        for(j = 0 ; j < result[i].num ; j++)
          expanded.push({w: result[i].w, h: result[i].h, type: result[i].type, area: result[i].w * result[i].h});
      }
      return expanded;
    },
    ...
    serialize: function(blocks) {
      ...
      for(i = 0; i < blocks.length ; i++) {
        block = blocks[i];
        str = str + block.w + "x" + block.h + "x" + block.type + (block.num > 1 ? "x" + block.num : "") + "\n";
      }
      return str;
    }
